...
var ArrowBodyContainer = function (_React$PureComponent) {
  ...
  ArrowBodyContainer.prototype.handleDelete = function handleDelete(target) {
    ...
    var currentIndex = this.props.entities.findIndex(function (entity) {
      return entity.id === _this4.props.from;
    });
    var currentTarget = _extends({}, this.props.entities[currentIndex]);
    var linkIndex = currentTarget.linksTo.findIndex(function (lk) {
      return lk.target === target;
    });
    if (linkIndex >= 0) {
      currentTarget.linksTo = [].concat(currentTarget.linksTo.slice(0, linkIndex), currentTarget.linksTo.slice(linkIndex + 1));
    }
